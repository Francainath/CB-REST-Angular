/*----------------------------------------------------------------------*/
/* wl_Multiselect v 1.3 by revaxarts.com
...
$.fn.wl_Multiselect = function (method) {
	...
	return this.each(function () {
		...
		if (!$this.data('wl_Multiselect')) {
			...
			$('<div class="comboselectbox"><div class="combowrap"><ul class="comboselect"></ul></div><div class="comboselectbuttons"><a class="add btn"><i class="icon-step-forward"></i></a><a class="addall btn"><i class="icon-fast-forward"></i></a><a class="removeall btn"><i class="icon-fast-backward"></i></a><a class="remove btn"><i class="icon-step-backward"></i></a></div><div class="combowrap"><ul class="comboselect"></ul></div></div>').insertAfter($this);
		...
		//add searchfield and its actions
		if(opts.searchfield){
			...
			}).prependTo($box.addClass('searchable').find('.combowrap').eq(0));
		}
		...
		//if items are set append them to the native input
		...
		$.each($this.find('option'), function (i, e) {
			var _this = $(this),
				name = _this.text(),
				value = _this.val();
			var item = $('<li class="btn"><a>' + name + '</a></li>').append('<a class="add"><i class="icon-plus"></i></a>').data({
				'pos': i,
				'value': value,
				'name': name,
				'native': _this
			...
			if (_this.is(':selected')) {
				...
				item.clone(true).data({
					'pos': j++
				}).attr('data-value', value).append('<a class="remove"><i class="icon-remove"></i></a>').appendTo(opts.selection).find('.add').remove();
				item.data('native').prop('selected', true);
				item.addClass('used');
				if (!opts.showUsed) item.addClass('hidden');
			}
		});
		...
		$add.bind('click.wl_Multiselect', function () {
			...
			if(opts.searchfield) $searchfield.focus();
		...
		$addall.bind('click.wl_Multiselect', function () {
			var selection = $.map(opts.pool.find('li').not('.hidden'), function (el) {
				return $(el).data('value');
			});
			if(opts.searchfield) $searchfield.val('').trigger('keyup').focus();
			$this.wl_Multiselect('select', selection);
		});
...
$.fn.wl_Multiselect.methods = {
	add: function (items, select) {
		...
		$.each(_items, function (value, name) {
			...
			//and elements
			var item = $('<li><a>' + name + '</a></li>').append('<a class="add"><i class="icon-plus"></i></a>').data({
				'pos': i++,
				'native': _native,
				'name': name,
				'value': value
	...
	select: function (values) {
		...
		if(values){
			...
			$.each(values, function (i, value) {
				...
				item.removeClass('selected').clone(true).attr('data-value', value).append('<a class="remove"><i class="icon-remove"></i></a>').appendTo(opts.selection).find('.add').remove();
				item.data('native').prop('selected', true);
				item.addClass('used');
				if (!opts.showUsed) item.addClass('hidden');
			});
	...
	unselect: function (values) {
		...
		if(values){
			...
			$.each(values, function (i, value) {
				...
				if (!opts.showUsed) item.removeClass('hidden');
			});
		}
